...
这个项目的重点不只是“写一个内存池”，而是把以下内容放在同一个实验框架里进行观察和验证：
...
- 性能结果对编译器、标准库实现、构建模式和测试 workload 非常敏感，不能简单写成“内存池一定更快”。

因此，本项目更准确的亮点是：通过双环境三轮 benchmark，拆分分析 allocator 与 container 对性能的影响，而不是单纯追求某一个固定平台下的加速数字。
