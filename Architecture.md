...
当前代码不统计进程私有内存，因此架构说明、结果文档和展示页都只保留耗时指标。
...
需要注意的是，Windows 下自定义组合相对 `std::vector` 更快，但同容器对比中 `SGIAllocator` 慢于 `std::allocator`，因此不能把 Windows 的整体收益简单归因为“内存池本身更快”。更准确的结论是：自定义内存池和简化版容器在不同平台、标准库实现和构建模式下表现差异明显，性能结论必须绑定测试环境。
